...
> **时间切片**不是 **React** 首创的，而是借鉴了计算机操作系统中的**单核 CPU** 的调度机制中的**「轮转」**策略
>
> 这是一种基于时钟的「**抢占策略**」: **公平地给每一个进程一定的执行时间，当时间消耗完毕或阻塞，操作系统就会调度其他进程，将执行权抢占过来**
>
> **决策模式**:
>
> - **抢占策略**：支持中断正在运行的进程，将主动权掌握在操作系统这里，不过通常开销会比较大
> - **非抢占策略**：非抢占策略指的是让进程运行直到结束、阻塞(如I/O或睡眠)、或者主动让出控制权
