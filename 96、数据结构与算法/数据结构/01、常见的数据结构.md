...
在计算机科学中，**数据结构**（**data structure**）是一种**==存储和组织数据==**的方式，目的是为了高效地**==访问==**和**==修改==**数据，数据结构的选择对算法的效率有很大的影响，可以极大地优化程序的性能
...
栈是一种遵从**==后进后出==**原则（**LIFO, Last In First Out**）的有序集合，新添加或带删除的元素都保存在站的同一端，称为**栈顶**，另一端就称为**栈底**；在栈里，新元素都靠近栈顶，旧元素都靠近栈底，这意味着最后被加入到栈中的元素会最先被移除
