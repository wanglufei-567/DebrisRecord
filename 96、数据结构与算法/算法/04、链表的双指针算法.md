...
**解析：** 这个链表分隔问题的解法采用了「**双链表构建**」策略 <!--非典型的双指针策略，但勉强算是双指针-->
...
<!--没有用到之前的归并双指针，但这道题是 3.1 合并两条链表的一种拓展，合并多条有序链表-->
...
因为一般不会知道链表总共有多少个节点，得先完整遍历一遍链表获取到总结点个数`n` ，那么倒数第 `k` 个节点就是正数第 `n - k + 1` 个节点，也就是说，这个解法需要遍历两次链表才能得到出倒数第 `k` 个节点

那么能不能**只遍历一次链表**，就算出倒数第 `k` 个节点？

假设 `k = 2`，思路如下：

- 首先，让一个指针 `p1` 指向链表的头节点 `head`，然后走 `k` 步

![img](https://raw.githubusercontent.com/wanglufei561/picture_repo/master/assets/1.jpeg)

- 现在的 `p1`，只要再走 `n - k` 步，就能走到链表末尾的空指针了

- 这时，再用一个指针 `p2` 指向链表头节点 `head`

![img](https://raw.githubusercontent.com/wanglufei561/picture_repo/master/assets/2-20250715013913187.jpeg)

- 接下来让 `p1` 和 `p2` 同时向前走
  - `p1` 走到链表末尾的空指针时前进了 `n - k` 步
  - `p2` 也从 `head` 开始前进了 `n - k` 步，停留在第 `n - k + 1` 个节点上，即恰好停链表的倒数第 `k` 个节点上

![img](https://raw.githubusercontent.com/wanglufei561/picture_repo/master/assets/3-20250715013922107.jpeg)
