...
<!--cookie、sessionStorage、localStorage 都只能存储字符串，若是大数据量的结构化数据可以用 indexDB-->
...
- 作为浏览器能力：
  ...
  - 浏览器限制了 **Cookie** 的跨域发送（受 `SameSite` 限制）<!--可以预防跨站请求伪造 CSRF-->

  - 可以通过 **JavaScript** 操作 **Cookie**（前提是未设置 `HttpOnly`）<!--可以预防跨站脚本攻击 XSS-->
