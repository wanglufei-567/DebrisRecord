...
- **解密**：==**接收方**使用**自己的私钥解密**消息==
  ...
  - 「客户端」想发送一条机密信息给 「服务端」
   ...
   <!-- 单向 https 认证的场景中，客户端就是用这个过程将自己的随机密钥（对称密钥）提供给服务端用于后续的对称加密-->
...
1. **SYN**：客户端发送一个 **SYN**（同步）包给服务器表示请求建立连接<!--我要建立连接-->
2. **SYN-ACK**：服务器收到 **SYN** 包后回复一个 **SYN-ACK**（同步-确认）包表示同意建立连接<!--同意连接-->
3. **ACK**：客户端收到 **SYN-ACK** 包后发送一个 **ACK**（确认）包表示连接建立成功<!--连接成功可以通信了-->
