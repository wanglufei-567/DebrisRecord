...
由于是**初次挂载**，所以新父  `fiber` 上是没有子 `fiber` 的，因此直接根据子 **VirtualDOM** 创建子 `fiber` 即可
...
- 若当前工作单元没有 `siblingFiber`，则将当前工作单元的父 `fiber` 赋值给给 `completedWork`，继续`completeUnitOfWork` 中的循环，完成父 `fiber` 的 `completeWork`
