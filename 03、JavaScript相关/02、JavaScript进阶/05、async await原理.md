...
在**JavaScript**中，任何数据结构只要部署**Iterator** 接口<!--在js中就是对象的一个属性Symbol.iterator--> ，那么便认为其是**可迭代对象**，可以进行遍历操作
...
==**生成器Generator**==是一种**==特殊类型的函数==**，它可以通过 **yield** 语句来**==暂停函数执行==**，并**==在需要时恢复执行==** <!--可以交出函数的执行权，即拥有暂停函数执行的效果-->
