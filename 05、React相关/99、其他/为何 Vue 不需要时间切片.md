...
旧版 **React** 架构采用递归的方式处理虚拟 **DOM** 树，一旦开始渲染就无法中断，会导致性能问题，特别是在复杂应用中会出现掉帧现象
...
- **响应式依赖追踪示意图**：
  ...
  **运作流程**：
  ...
  1. 组件初次渲染时，**渲染 Watcher **读取模板中的响应式数据
...
2. **React** 的时间切片机制**
   ...
   - **目的**：确保在浏览器典型 60**FPS**（约 16 毫秒一帧）的渲染时间内，可以平滑地分散任务，保持界面流畅性
...
1. **优势领域**：
   - 中小型应用的开发效率
   - 自动化的依赖追踪
   - 更小的运行时体积（**Vue** 3 核心仅 30**KB**）
   - 大多数场景下的更新耗时低（<1**ms**），无需时间切片
