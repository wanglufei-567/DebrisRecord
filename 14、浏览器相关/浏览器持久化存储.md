...
浏览器提供了多种**「持久化存储机制」**，用于在客户端存储数据

以下是目前主流浏览器支持的**「持久化存储方式」汇总**，按能力、容量和适用场景分类：

| 存储方式                 | 容量限制          | 是否持久化 | 同步/异步 | 跨页面共享    | 数据类型       | 适用场景                       |
| ------------------------ | ----------------- | ---------- | --------- | ------------- | -------------- | ------------------------------ |
| `Cookies`                | ~4KB              | ✅ 是       | 同步      | ✅ 跨请求/页面 | 字符串         | 会话管理、跨请求携带身份       |
| `sessionStorage`         | ~5MB              | ❌ 否       | 同步      | ❌ 不共享      | 字符串         | 会话级数据，页面跳转临时保存   |
| `localStorage`           | ~5MB              | ✅ 是       | 同步      | ✅ 同域共享    | 字符串         | 小量键值数据，用户偏好等       |
| `IndexedDB`              | 数十 MB ~ 几百 MB | ✅ 是       | 异步      | ✅ 同域共享    | 任意结构化数据 | 离线缓存、大数据、复杂对象存储 |
| `Cache Storage`          | 数百 MB           | ✅ 是       | 异步      | ✅ 同域共享    | 响应对象       | PWA 离线缓存（Service Worker） |
| `File System Access API` | 几百 MB~GB        | ✅ 是       | 异步      | ✅（需授权）   | 文件/目录      | 直接读写本地文件（需权限）     |
