...
1. ##### 解析 URL 生成 HTTP 请求信息
   ...
   > **HTTP 协议**定义了客户端和服务端交互的信息「内容」和「步骤」
   >
   ...
   1. **浏览器**先对 **URL** 做**语法解析**
      ...
      - scheme（如 `http`、`https`）
      - host（域名或 IP）
      - port（若未写，按协议默认：HTTP 80、HTTPS 443）
      - path / query / fragment 等

      <!--此时浏览器还没有进行“DNS 查询”，只是把 URL 拆解成结构化信息-->

   2. **浏览器检查本地资源缓存、DNS 缓存等**

      1. **检查资源缓存（Memory Cache / HTTP Cache）**

         - 如果缓存命中且未过期 → **直接使用缓存**，不需要 DNS
         - 如果缓存失效或不存在 → 需要发起网络请求

      2. **DNS 查询（浏览器缓存 → OS 缓存 → 真实 DNS 服务器）**

         - 浏览器先查自己的 DNS 缓存
         - 再查 OS DNS 缓存

         <!--如果这些地方已经有对应域名的 IP，则直接使用，无需再发起 DNS 查询-->

         - 如果没有命中，再发 DNS 请求

   3. **如果本地没有，则由操作系统负责 DNS 查询**

   > **可以总结为：**浏览器解析 URL → 检查 IP 缓存 → 若无则委托 OS 做 DNS 解析 → 得到 IP → 建立连接 → 发送请求

2. ##### DNS 域名解析

   1. 浏览器调用 **Socket 库**中的 **DNS解析器**查询 **IP地址**
      - 查询 **IP 地址**其实也是一次网络通信，是 **Socket 库**和 **DNS 服务器**的通信
      - ==**DNS 服务器**的 **IP 地址**已经配置在系统里面了==，传输层的协议使用的是 **UDP 协议**而不是 **TCP 协议**
...
3. ##### 委托协议栈发送消息 <!--其实就是建立TCP连接，根据IP地址和默认80端口，和服务器建立TCP连接-->
   ...
   2. **CSSOM** 树的构建
         ...
         - 在解析 **HTML** 文档的过程中
           ...
           - 当浏览器遇到 `<style>` 标签，会立即开始解析 **内联样式**
           - 当浏览器遇到 `<link>` 标签时，会立即开始下载并解析 **CSS 文件**
   ...
   3. **JavaScript** 文件的加载与执行
         ...
         - 当浏览器遇到 `<script>` 标签（用于加载或内联 **JavaScript**）时，行为取决于脚本的类型和属性：
           ...
           - `async` 属性：浏览器会继续解析 **HTML**，同时异步加载 **JavaScript** 文件，一旦文件加载完成，浏览器会立即暂停解析并执行该脚本
           
           - `defer` 属性：浏览器会继续解析 **HTML**，直到文档解析完成才会执行 **defer** 脚本，这种方式不会阻塞页面的渲染，并且多个 **defer** 脚本会按照它们在 **HTML** 中的顺序执行
