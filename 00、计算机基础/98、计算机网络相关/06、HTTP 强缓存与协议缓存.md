...
- **强缓存（Strong Cache）**
  ...
  - ==**「强缓存」**的命中规则本质上就是检查**是否在有效期内**，不会验证资源内容变化==

  - **关键响应头字段**：
    - `Cache-Control: max-age=xxx`
      - 计算方式：（当前时间 - `responseTime`） < `max-age`
        - `responseTime`：浏览器收到该资源的时刻，保存在浏览器缓存的元信息中
        -  `max-age`：服务端返回的有效期秒数
    - `Expires`
      - 计算方式：当前时间 < `Expires` 指定时间
        - `Expires` 是一个 **绝对时间**，浏览器需要直接对其和比当前时间

- **协商缓存（Negotiated Cache）**
  - **当强缓存失效后**，浏览器会向服务器**发送请求并附带缓存标识**（如 `If-None-Match` 或 `If-Modified-Since`），服务器根据这些标识判断资源是否更新 
    - 若资源**未变更**，返回 `304 Not Modified`，浏览器继续使用缓存

    - 若资源**已变更**，返回 `200 OK` 和新的资源，浏览器更新缓存副本

  - ==**「协商缓存」**的命中规则，本质上就是 **服务端判断“内容是否变化”**==
  - 关键响应头字段：
    - `ETag` ：服务端资源的**唯一标识**
    - `Last-Modified` ：服务端资源的上次修改时间，单位是秒
  - 关键请求头字段：
    - `If-None-Match`：
      - 之前请求返回的 `ETag` 响应头字段
      - 保存在缓存数据的元信息中，浏览器自动携带 <!--相当于问服务端：“文件内容变了吗？”-->
    - `If-Modified-Since`
      - 之前请求返回的 `Last-Modified` 响应头字段
      - 保存在缓存数据的元信息中，浏览器自动携带 <!--相当于问服务端：“修改时间变了吗？”-->

无论是「**强缓存**」还是「**协商缓存**」，本质上缓存的都是服务器返回的「**响应内容**」<!--响应体 + 响应头-->

### 二、缓存处理流程机制

1. **浏览器首次请求资源**
   - 服务器返回资源内容，并设置缓存相关响应头（如 `Cache-Control`,`ETag`,`Last-Modified`）
2. **浏览器再次请求资源，流程如下：**
...
| 字段名          | 类型   | 说明                                                   |
| --------------- | ------ | ------------------------------------------------------ |
| `Cache-Control` | 响应头 | 时间范围，控制是否启用缓存、缓存时长等策略（HTTP/1.1） |
| `Expires`       | 响应头 | 绝对时间，缓存过期时间（HTTP/1.0，已被替代）           |
...
在实际应用中，最终生效的缓存策略通常以响应头中的 `Cache-Control` 为准，因为==服务器的指令优先级更高==
