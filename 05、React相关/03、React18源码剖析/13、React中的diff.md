...
> 不做任何优化的情况下，**diff 算法**的复杂度是 **O(n³)** ：
>
> 1. **每层子节点匹配**：如果父节点有 **n** 个子节点，没有 **key**，比较每个新子节点都要遍历旧子节点 → 每层 **O(n²)**
> 2. **树的高度**：虚拟 **DOM** 树高度为 **h**，每层都要做这样的比较 → **总复杂度 O(n² × h)**
> 3. **最坏情况假设**：**h** 与 **n** 同阶（例如链状树或宽度接近 n 的树） → **O(n² × n) = O(n³)**
>
> 不过 **O(n³)** 是**理论最坏情况**，在没有任何优化的树 **diff** 算法下才会出现
>
>  **React** 通过 **key**、**分层比较**、**类型判断**等优化，将实际复杂度降到接近 **O(n)**
