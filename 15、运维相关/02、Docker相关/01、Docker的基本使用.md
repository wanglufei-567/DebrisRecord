...
> 操作系统分为内核和用户空间，对于 **Linux** 而言，内核启动后，会挂载 **root 文件系统**为其提供「用户空间」支持，容器创建的**image** 就相当于是一个 **root 文件系统**
>
> 用户空间是一个应用程序运行的环境，它包括以下内容：
>
> - 文件系统
> - 应用程序依赖的库和工具
> - **Shell** 等工具
>
> 这些都不是内核的一部分，而是「操作系统」的「用户空间组件」，即使容器使用的是宿主机的「内核」，容器内的应用程序**仍然需要**这些用「户空间组件」来运行
