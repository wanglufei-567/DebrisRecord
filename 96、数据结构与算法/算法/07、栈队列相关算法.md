...
function nextGreaterRight(nums) {
  ...
  for (let i = 0; i < n; i++) {
     ...
     *
     * 这里是找右侧第一个更大元素
     * 若当前元素大于栈顶元素，则弹出栈顶元素，说明栈顶元素的下一个更大元素就是当前元素
     * 这样就找到了栈顶元素的第一个更大元素
     *
    ...
    while (stack.length && nums[i] > nums[stack[stack.length - 1]]) {
      // 弹出栈顶元素，
      const idx = stack.pop();
      // 当前元素就是栈顶元素的下一个更大元素，将当前元素记录到栈顶元素对应的索引位置
      res[idx] = nums[i];
    }
...
- 遍历到 2 时，栈为空，将 2 压入栈
- 遍历到 1 时，1 小于栈顶元素 2，将 1 压入栈
- 遍历到 2 时，2 大于栈顶元素 1
  - 说明栈顶元素 1 的下一个更大元素就是 2
  - 将栈顶元素 1 弹出，记录下其下一个更大元素2
  - 同时将当前元素 2 压入栈
- 遍历到 4 时，4 大于栈顶元素 2
  - 说明栈顶元素 2 的下一个更大元素就是 4
  - 将栈顶元素 2 弹出，记录下其下一个更大元素4
  - 同时将当前元素 4 压入栈
- 遍历到 3 时，3 小于栈顶元素 4
  - 说明栈顶元素 4 的下一个更大元素就是 3
  - 将栈顶元素 4 弹出，记录下其下一个更大元素3
  - 同时将当前元素 3 压入栈
- 最后返回 [4,2,4,-1,-1]
- -1表示对应索引位置的元素不存在下一个更大元素，同时也说明单调栈到最后还没清空
